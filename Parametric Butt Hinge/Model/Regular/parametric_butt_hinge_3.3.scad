...
//
// - Version 3.2
//   * Added support for independantly configurable top and bottom pin shaft counterbores.
...
m_bottom_pin_shaft_counterbore_depth    = clip ( bottom_pin_shaft_counterbore_depth, C_MIN_PIN_SHAFT_COUNTERBORE_DEPTH, 0.66*( m_leaf_height / ( m_knuckle_count + m_component_clearance )) );
...
module tool_cutter_pin_shaft_counterbore ( diameter_top, depth_top, shape_top, diameter_bottom, depth_bottom, shape_bottom )
{
    ...
    echo ( shape_bottom = shape_bottom );
    echo ( shape_top    = shape_top );
    ...
    if ( BOTTOM_COUNTER_BORE_ENABLED )
    {
        ...
        {
            translate ( [ xd, yd0, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h0, r = d0/2, center = true );
        }
    ...
    {        
        ...
        {
            translate ( [ xd, yd1, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h1, r = d1/2, center = true );
        }
