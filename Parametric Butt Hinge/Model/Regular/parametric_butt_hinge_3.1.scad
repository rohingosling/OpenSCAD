...
//       peiece configurations that have their pin disabled. For instance, when the hinge is to be assembled using an external pin.
...
//       in the 'leaf' module to 'true', which will enable a single row of inscribed text on each lead.
...
pin_shaft_counterbore_diameter = 7.0;
pin_shaft_counterbore_depth    = 2.5;
pin_shaft_counterbore_shape    = 0;            // [ 0:Circular, 1:Square, 2:Hexagonal ]
...
m_pin_shaft_counterbore_diameter = clip ( pin_shaft_counterbore_diameter, C_MIN_PIN_SHAFT_COUNTERBORE_DIAMETER, 2.0*m_leaf_gauge - C_MIN_PIN_SHAFT_COUNTERBORE_WALL_THICKNESS );
m_pin_shaft_counterbore_depth    = clip ( pin_shaft_counterbore_depth, C_MIN_PIN_SHAFT_COUNTERBORE_DEPTH, 0.66*( m_leaf_height / ( m_knuckle_count + m_component_clearance )) );
m_pin_shaft_counterbore_shape    = pin_shaft_counterbore_shape;
...
{  
    ...
    rotate ( [ 0, 0, gender_angle ] )
    {
        ...
        difference ()
        {
            ...
            // Cut pin shaft counterbore.
            
            if ( m_pin_shaft_counterbore_enabled )
            {
                tool_cutter_pin_shaft_counterbore ( diameter = m_pin_shaft_counterbore_diameter, depth = m_pin_shaft_counterbore_depth, shape = m_pin_shaft_counterbore_shape );
            }
...
module tool_cutter_pin_shaft_counterbore ( diameter, depth, shape )
{       
    // Local constants.
    ...
    x = d/2.0;              // Run of the right angle triangle who hypotenuse is equal to half the radius of our new circle.
    y = x*tan ( a );        // Rise of the right angle triangle who hypotenuse is equal to half the radius of our new circle.
    r = sqrt ( x*x + y*y ); // Hypotenuse of the right angle triangle who hypotenuse is equal to half the radius of our new circle.
    ...
    xd = 0.0;
    yd = ( h - m_leaf_height )/2.0 - SCG_OVERLAP;
    zd = 0.0;
    ...
    if ( shape == C_CIRCULAR )
    {
        translate ( [ xd,  yd, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h, r = d/2, center = true );
        translate ( [ xd, -yd, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h, r = d/2, center = true );
    }
    ...
    if ( shape == C_SQUARE )
    {
        translate ( [ xd,  yd, zd ] ) cube ( [ diameter, h, diameter ], center = true );
        translate ( [ xd, -yd, zd ] ) cube ( [ diameter, h, diameter ], center = true );
    }
    
    color ( "red" )
    if ( shape == C_HEXAGONAL )
    {   
        translate ( [ xd,  yd, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h, r = r, center = true, $fn = s );
        translate ( [ xd, -yd, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h, r = r, center = true, $fn = s );
    }
}
